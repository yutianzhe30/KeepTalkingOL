...
系统会从以下 **8 个固定迷宫**中随机选择一个（简单模式固定使用 #1）。

所有迷宫均为 **8×8 网格**，坐标列(x)从左到右为 0–7，行(y)从上到下为 0–7。

**关键**：每个迷宫的**起点位置唯一**。让拆弹者报告红点坐标，立即对应到唯一地图：
...
#### 迷宫地图 #1（起点 0,3 左边 → 出口 7,4 右边）难度：简单
```
┌───┬───┬───┬───┬───┬───┬───┬───┐
│ █ │ █ │ █ │ █ │ █ │ █ │ █ │ █ │
│ █ │   │ █ │   │   │ █ │   │ █ │  第1行
...
#### 迷宫地图 #2（起点 4,0 上边 → 出口 3,7 下边）难度：中等
```
┌───┬───┬───┬───┬───┬───┬───┬───┐
│ █ │ █ │ █ │ █ │ S │ █ │ █ │ █ │  ← 起点
│ █ │   │   │ █ │   │   │ █ │ █ │  第1行
...
#### 迷宫地图 #3（起点 0,1 左边 → 出口 6,7 下边）难度：中等
```
┌───┬───┬───┬───┬───┬───┬───┬───┐
│ █ │ █ │ █ │ █ │ █ │ █ │ █ │ █ │
│ S │   │ █ │ █ │ █ │ █ │ █ │ █ │  第1行 ← 起点
...
#### 迷宫地图 #4（起点 7,2 右边 → 出口 0,6 左边）难度：困难
```
┌───┬───┬───┬───┬───┬───┬───┬───┐
│ █ │ █ │ █ │ █ │ █ │ █ │ █ │ █ │
│ █ │   │   │   │ █ │   │   │ █ │  第1行
...
#### 迷宫地图 #5（起点 2,0 上边 → 出口 5,7 下边）难度：中等
```
┌───┬───┬───┬───┬───┬───┬───┬───┐
│ █ │ █ │ S │ █ │ █ │ █ │ █ │ █ │  ← 起点
│ █ │   │   │   │ █ │   │   │ █ │  第1行
...
#### 迷宫地图 #6（起点 0,4 左边 → 出口 7,3 右边）难度：中等
```
┌───┬───┬───┬───┬───┬───┬───┬───┐
│ █ │ █ │ █ │ █ │ █ │ █ │ █ │ █ │
│ █ │   │   │   │ █ │   │   │ █ │  第1行
...
最短路径 (10步): 东→东→南→东→东→北→东→北→东→东
```

#### 迷宫地图 #7（起点 1,7 下边 → 出口 6,0 上边）难度：困难
```
┌───┬───┬───┬───┬───┬───┬───┬───┐
│ █ │ █ │ █ │ █ │ █ │ █ │ E │ █ │  ← 出口
│ █ │   │ █ │   │ █ │   │   │ █ │  第1行
...
最短路径 (12步): 北→东→北→东→北→北→北→东→东→北→东→北
```

#### 迷宫地图 #8（起点 7,5 右边 → 出口 0,2 左边）难度：困难
```
┌───┬───┬───┬───┬───┬───┬───┬───┐
│ █ │ █ │ █ │ █ │ █ │ █ │ █ │ █ │
│ █ │   │   │   │ █ │   │ █ │ █ │  第1行
...
> [!TIP]
> **快速识别迷宫**：
> 起点坐标在所有地图中**唯一**，拆弹者一报坐标即可确定地图，无需试探。

> [!CAUTION]
> **常见错误**：
> *   走偏后没有及时利用撞墙闪烁重新校准，步数和方向对不上了。
> *   起点在边缘，不要误以为还有更外层的墙。

### 确认到达
当拆弹者的**红点**和**绿点**重叠时，模块自动解除（无需额外按钮）。

---

### 专家提示
*   **让拆弹者边走边报告**：每走一步都说"走了"，这样你可以跟踪步数。
*   **撞墙有红色闪烁**：可以用来验证当前位置和路径是否正确。
*   **出口可见范围**：只有在距离出口 **3 格以内**时，绿点才会显示。更远时只能靠地图导航。
*   **方向约定**：东=右，西=左，南=下，北=上。
