...
此模块中，拆弹者**完全看不到迷宫的墙壁**，只能看到自己和出口的位置（如果有的话）。你必须通过语言导航，指挥拆弹者走出迷宫。
...
**重要**：首先让拆弹者报告初始位置（绿点坐标），然后对照下表找到对应的迷宫地图。
...
最短路径 (7步): 南→东→东→南→南→东→东
参考路径 (8步): 东→南→东→东→南→南→东→东
```
...
最短路径 (13步): 南→东→北→东→南→南→东→东→南→西→南→东→东
这是最长路径迷宫，需要耐心！
```
...
最短路径 (11步): 东→东→南→南→西→南→东→东→北→东→南
```
...
> *   忘记确认初始朝向，导致方向完全相反。
> *   拆弹者走偏后没有及时用撞墙声校准位置。
> *   距离出口太远时看不到红点，需要靠步数估算。

### 第三阶段：确认到达
当拆弹者的绿点和红点**重叠**时，让拆弹者点击 **「确认到达」** 按钮。
...
*   **撞墙不是坏事**：可以用来校准当前位置。
*   **出口可见范围**：只有在距离出口 3 格以内时，红点才会显示。在此之前只能靠迷宫地图导航。
